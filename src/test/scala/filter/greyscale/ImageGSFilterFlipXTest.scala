...
class ImageGSFilterFlipXTest extends AnyFunSuite {

  test("Image with multiple rows and columns") {
    val filter = new ImageGSFilterFlipX()
    ...
    val expected: Seq[Seq[PixelGS]] = Seq(
      ...
      Seq(PixelGS(9), PixelGS(8), PixelGS(7))
    )
  ...
  test("Image with a row of pixels") {
    ...
    val expected: Seq[Seq[PixelGS]] = Seq(
      Seq(PixelGS(4), PixelGS(3), PixelGS(2), PixelGS(1))
    )
    val expectedImage: Image[PixelGS] = ImageGS(expected)
